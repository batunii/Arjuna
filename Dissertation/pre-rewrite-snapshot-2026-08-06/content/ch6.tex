...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{content/figures/fig_block_a_layout.png}
...
clip.} That is 1,040 marker presentations in total across the thirteen usable pairs collected so far.
...
published gaze landmarks rather than chosen after seeing the data.

The bands are placed where the driving literature already locates the safety-critical periphery.
Driver gaze concentrates in a road-centre region of roughly 8$^{\circ}$ radius \citep{victor2005}, so
the under-10$^{\circ}$ band covers what a viewer is looking at anyway, and it is the region where a
guidance effect has least to add. The Peripheral Detection Task, the standard instrument for
measuring spare attentional capacity under load, deliberately places its probes just outside that
region, at 11–23$^{\circ}$ horizontal eccentricity \citep{jahn2005}, and the Detection Response Task
standardises the same logic with a fixed, background-independent stimulus \citep{iso2016}. The
10–20$^{\circ}$ and 20–30$^{\circ}$ bands bracket that published range. The outer boundary is the
conventional useful-field-of-view extent of roughly 30$^{\circ}$ \citep{ball1993}, past which a target
falls outside the radius from which people extract task-relevant information without moving their
eyes, so a stronger peripheral signal there has little left to buy. H2b's safety check pools hit rate
across all four bands, that is, across the whole field of view.

One point of fit is worth stating precisely, because the 20--30$^{\circ}$ band carries H2a. The
published probe range stops at 23$^{\circ}$, so it covers the 10–20$^{\circ}$ band completely but only
the lower part of 20–30$^{\circ}$. The boundaries here are round numbers chosen to bracket the
published landmarks, not to reproduce any one study's geometry.
...
Distractor floor (distractors fail to distract) & Relies on the established distractor-cost literature cited in Section 6.1.1 \citep[N = 66]{ai2025} rather than an internal manipulation check; not independently verified within this study \\
...
unpinned. Figure~\ref{fig:h1} shows every participant's pair.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{content/figures/fig_slopegraph_h1.png}
\caption{Block A per-participant accuracy, vignette off versus
Hard Dark on (N = 14).}
\label{fig:h1}
\end{figure}
...
\caption{Block B pooled hit rate against
...
Figure~\ref{fig:h2a} shows the four bands against the gate strength that predicts them.
...
overlaid (H2a, N = 13). The upper panel pools every marker presentation in a band. The lower panel
...
shows no loss at all. Chapter 7 takes up what to make of that gap. Figure~\ref{fig:esq} shows the
three constructs side by side.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{content/figures/fig_questionnaire.png}
\caption{End-of-session questionnaire responses by construct (N = 13).}
\label{fig:esq}
\end{figure}
