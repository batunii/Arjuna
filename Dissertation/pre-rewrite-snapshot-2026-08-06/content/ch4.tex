...
Figure~\ref{fig:fusion} contrasts the two sampling schemes on the same world point.
...
content is present but muted. The floor sits at 0.4 rather than lower because a periphery approaching
black stops carrying peripheral events at all, and a monitoring mode whose own dimming floors
peripheral detection would fail the safety hypothesis of Chapter 6 by construction rather than by
finding. This track covers the great majority of peripheral pixels, so its floor, rather than the
signal track's, is what decides whether the periphery stays usable at all.

\textbf{The ROG track}: inside the window, saturation is boosted $\times$\verb|_PopSatBoost| (1.7) about the pixel's
own luminance, a sigmoidal midtone-contrast transfer is applied per \citet{sutton2022}'s published recipe
...
Figure~\ref{fig:colorpop} traces a fragment through the classification, the two grading tracks, and
the periphery dim that merges them.
...
luminance edges restored (\textbf{OutlinedDark}, after \citealp{cheng2022}'s finding that
...
\item \textbf{A darkened surround.} A soft annulus just outside the object's boundary is dimmed
  ...
  (the dual modulation of \citealp{veas2011}, the same source cited for ConspicuitySqueeze,
  Section 4.5.3).
\end{itemize}
...
is treated as re-engagement, so it returns promptly. Figure~\ref{fig:suppression} shows the threshold,
the hold timer, the two fade ramps, and the focus-arrival shortcut as one state diagram.
...
constant. Figure~\ref{fig:interaction} follows one placement from painting to locked formation.
...
one arm of the planned oracle-versus-live benchmark. Figure~\ref{fig:oracle} shows the pipeline from
source video to deterministic playback.
