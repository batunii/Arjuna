...
the taxonomy that follows. Figure~\ref{fig:stack} shows the three layers of the stack and which of
them each tier of access is permitted to touch.
...
Table~\ref{tab:tiers} and elaborated below.
...
paths, latency, thermal envelope, is the subject of Chapter 5. Figure~\ref{fig:hybrid} separates the
composite into its layers and shows the single view they fuse into.
...
at three points. Figure~\ref{fig:axes} places the three on both axes, with deployability shown
alongside.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{content/figures/fig_design_axes.png}
...
of the kind demonstrated by \citet{sutton2022}, plus whatever dimming hardware exists. Second, the \textbf{human-factors
