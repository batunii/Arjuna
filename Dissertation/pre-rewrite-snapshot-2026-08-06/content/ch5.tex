...
\item \textbf{Perfetto system traces}, captured via the Meta developer tooling (\verb|metavr| CLI / Perfetto
  ...
  \citep{meta2025a}.
\item \textbf{OVR Metrics Tool overlay and logcat telemetry} \citep{meta2025b} for sustained frame rate,
...
\begin{longtable}{p{0.05\textwidth}|p{0.44\textwidth}|p{0.15\textwidth}|p{0.24\textwidth}}
...
matter, and CPU main-thread and render-thread times are extracted alongside GPU time, because an
overlay sphere is cheap on the GPU while the manager scripts spend on the CPU.

\textbf{Metrics.} Median and 95th-percentile GPU frame time (ms), median CPU main and render thread
time (ms), stale-frame rate (\%), and headroom against the display budget (13.9 ms at 72 Hz, 11.1 ms
at 90 Hz).

\textbf{Criteria.} \textit{Pass} for a configuration used in the user study (2, 3, 5, 6): 95th-percentile GPU
frame time within budget and stale-frame rate < 1\%, because jitter in a study stimulus is a
confound, not merely a defect. Configuration 4 (Blur) is an \textit{interpretation band} rather than a
pass/fail: its cost relative to configuration 2 is the price of Tier-3 camera re-rendering against
Tier-2 overlay compositing, which is a design-space datum in its own right.

\figplaceholder{60mm}{Bar chart: median and p95 GPU frame time per configuration against the frame-budget line.}{Per-mode GPU frame time against the frame budget (T1).}
...
short of natural vision acuity \citep{wang2026}. This benchmark measures the
...
\textbf{Method limitation, stated in advance.} A camera at the eyepiece is not a human eye, so
absolute acuity values from this method are indicative rather than clinical. The claim structure is
therefore \textit{relative}: same camera, chart, lighting, and placement across all three conditions, so
the window-versus-periphery \textit{gap} is meaningful even where the absolute values are not.

\textbf{Criteria.} \textit{Interpretation, not pass/fail:} the architecture is vindicated if the window path
resolves at least one optotype line finer than the camera-feed path at 0.6 m. If the two measure
equal, the window-as-absence design loses its quality argument, though it retains the FOV and
latency arguments, and the dissertation will say so.

\figplaceholder{60mm}{Side-by-side through-lens crops of the same chart region: native window vs Tier-3 periphery vs no-headset reference.}{Side-by-side through-lens crops of the same chart region.}
...
One clarification keeps this benchmark honest. The user study uses no \textit{live} detection anywhere.
SignPop's colour-pop gate, its local window, and its saliency boost (Chapter 4, Section 4.5.5) are
all driven by the offline oracle track baked in advance (Section 5.8). This benchmark therefore
measures how far a \textit{live} version of the exact mechanism the study tests would fall short of the
conditions participants actually experienced, which is the size of what would have to close before
the human-factors finding could be claimed for a deployed system.

\textbf{Method.} The deployable on-device configuration (YOLO11n, \citealp{jocher2024}, via Unity Sentis, \citealp{unity2024}, FP16, at the
...
\begin{longtable}{p{0.37\textwidth}|p{0.20\textwidth}|p{0.23\textwidth}|p{0.08\textwidth}}
...
\textbf{Matching procedure.} Frame correspondence is exact, since both pipelines index the same video
timestamps, so matching reduces to greedy per-frame box assignment by descending IoU, each oracle box
matched at most once. Unmatched live boxes count as false positives and unmatched oracle boxes as
misses. Live confidence thresholds will be swept and the operating point reported, so the headline
recall is not an artefact of one threshold.

\textbf{Threat to this benchmark, inherited by its consumers.} The oracle is full-resolution YOLO11,
not human annotation, so it is a \textit{reference} whose own errors are unknown. Treating it as ground
truth is defensible for measuring the gap between the offline and on-device pipelines, because both
face the same footage and the offline one strictly dominates in resolution and compute. The absolute
detection quality of either against human-verified truth is not established here, and no claim in
this dissertation depends on it.
...
qualifier. The small-target row exists because distant traffic lights are precisely where a
resolution-limited pipeline fails first (Section 5.8).
...
measured on-device run \citep{laghari2025}. The present system's Tier-3 path is
...
Three rules bind the results revision of this chapter. Results will be reported for every benchmark
attempted, including failures and anomalies, with trace archives retained. No criterion or band
stated above will be adjusted after the data are seen. And where a benchmark is dropped for time,
the pre-agreed sacrifice order putting T5 first, the drop is stated rather than silently omitted.
