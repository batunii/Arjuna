...
In the Mahābhārata, Droṇa, the martial teacher of the Pāṇḍava and Kaurava princes, sets up a wooden 
...
Droṇa asks Arjuna the same question, then asks again: does he see the tree? The branches? Arjuna answers
...
recent study of sixty-six participants (Ai et al., 2025), replicating decades
...
Cheng et al., 2022),  and its application to distraction has recently been named \textit{visual noise
...
First, the study's dynamic scenario runs as a \textit{simulation}: pre-recorded driving footage presented on
...
deliberately decoupled from the attention question, but from \textit{live, on-device} detection
specifically, not from detection itself. SignPop, the mode evaluated in the driving block, is not
detector-free: its signal pop, a detection aperture opened over the object, a saturation and brightness
lift, and a darkened surround are all gated on a single detection signal (Chapter 4, Section 4.5.5).
What makes the decoupling real is where that signal comes from, an \textit{oracle}, computed once,
offline, at full resolution over the study footage (Chapter 5, Section 5.8), never from a model
...
Chapter 5's Benchmark T4 measures the size of the gap between that coverage and what a live pipeline
...
The primary, workstation, block runs on \textit{real passthrough with real physical distractors}: the
actual artefact, no simulation. The secondary, driving, block evaluates the concept in the
...
\item \textbf{C4: Pre-registered user study, with interim results.} A falsifiable within-subjects design
  ...
  pilot gates, and a pre-committed decision table, together with an interim analysis of the eleven
  participant pairs collected so far: a workstation-focus benefit (+3.65 points accuracy), no loss of
...
design in full and reports interim results from the eleven participant pairs collected so far.
...
\textit{Figure 1.1: The three peripheral treatments seen from the user's viewpoint: SignPop, Soft Dark,
Hard Dark, each with the world-locked focus window visible. Composite of headset screenshots.}

\textit{Figure 1.2: The two-part structure of the dissertation: Part T (design space $\rightarrow$ system $\rightarrow$ benchmarks)
...
\refitem{Biocca, F., Tang, A., Owen, C., \& Xiao, F. (2006). Attention funnel: omnidirectional 3D cursor for
mobile augmented reality platforms. \textit{Proc. CHI 2006}. \url{https://ieeexplore.ieee.org/document/1579336/}}

\refitem{Cheng, Y., Yin, H., Yan, Y., Gugenheimer, J., \& Lindlbauer, D. (2022). Towards understanding
diminished reality. \textit{Proc. CHI 2022}. \url{https://dl.acm.org/doi/10.1145/3491102.3517452}}

\refitem{Itti, L., Koch, C., \& Niebur, E. (1998). A model of saliency-based visual attention for rapid scene
analysis. \textit{IEEE TPAMI, 20}(11).}

\refitem{McLaughlin, A. C., et al. (2025). Cognitive aid design using diminished reality to support selective
attention by reducing distraction. \textit{Human Factors}, 67(9), 937–961. doi:10.1177/00187208251325169}

\refitem{Mori, S., Ikeda, S., \& Saito, H. (2017). A survey of diminished reality: techniques for visually
concealing, eliminating, and seeing through real objects. \textit{IPSJ T-CVA, 9}(17).
\url{https://link.springer.com/article/10.1186/s41074-017-0028-1}}
...
\refitem{Hong, J., Langlotz, T., Sutton, J., \& Regenbrecht, H. (2024). Visual Noise Cancellation: Exploring
Visual Discomfort and Opportunities for Vision Augmentations. \textit{ACM Transactions on Computer-Human
