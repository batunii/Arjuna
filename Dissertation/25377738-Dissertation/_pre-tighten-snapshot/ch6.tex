...
Twelve people have been tested across an initial pilot day and three further collection days,
preceded by informal, naive-pilot iteration used to tune stimulus parameters and task form ahead of
the frozen protocol. Eleven usable pairs exist for each of the two confirmatory blocks. Section 6.11
reports the interim results
from those eleven pairs, explicitly as interim: recruitment continues toward the N = 20 target, and
nothing in Section 6.11 is the pre-registered confirmatory analysis this chapter specifies. The
operational source document for this chapter is \verb|testing-strategy-v2.md|. Where this chapter
compresses, that document remains normative.
\end{quote}
...
\textit{live, on-device} detection specifically, not from detection itself}. SignPop, Block B's
evaluated mode, is not detector-free: its colour pop, a detection aperture opened over the detected
object, and a saliency lift are all gated on a single detection signal (Chapter 4, Section 4.5.5).
That signal comes from an oracle, the offline full-resolution detection bake, computed once, in
advance, never from a model running during a session. Every stimulus is otherwise deterministic and
scripted. The oracle is used exactly as an oracle is meant to be used: it lets the study ask whether
an effect built on excellent detection helps attention, without first building excellent detection
on-device. The human-factors finding therefore does not depend on today's on-device detection
quality. But it does depend on oracle-quality coverage, and Chapter 5's oracle-vs-live benchmark
measures the size of the gap between that coverage and what a live pipeline currently achieves,
which conditions how far the finding travels toward a deployed system. Nor does the current design
distinguish which of the gated effects, the colour pop, the opened window, or the saliency lift,
is responsible for any measured benefit. Section 6.10.2 carries this forward as a stated limitation.
...
will contribute 140 scored task events per Block A condition and 40 marker events per Block B
condition. Twenty participants across two Block A conditions therefore yield on the order of 5,600
Block A trials, a dataset that trial-level mixed models can exploit fully, while
...
lock-in memo (Section 6.8.2). H2b's 10-percentage-point margin reflects the safety framing inherited
from the DR focus-versus-awareness trade-off documented by McLaughlin et al. (2025) and Murph et al.
(2021): in a monitoring context, losing more than one detection event in ten to the overlay,
pooled across the whole scene, is an unacceptable awareness cost. Equivalence testing follows Lakens
(2017), which gives the safety hypothesis a form that can fail rather than a bare null.
...
distractors, are unchanged. Items requiring a wording check before use include the virtual
task panel (if the pilot selects it over the approved paper task), audio recording of the
end-of-session questionnaire's open items (experimenter notes are the fallback), and the stated
session duration on the information sheet.

Sickness risk is managed by design and by rule. By design: all tasks are seated, there is no
artificial locomotion, total headset time is approximately 43 minutes, and a headset-off break separates the
two confirmatory blocks. By rule, a written \textbf{stop rule} is applied without discussion: the VR
...
check precedes headset removal (Section 6.7). Additional checks occur between any conditions
on request. After termination the participant rests seated, is offered water, and remains until
symptoms subside. Data collected up to termination are retained per the consent wording, and the
participant is replaced from the spare pool. The Virtual Reality Sickness Questionnaire (VRSQ; Kim et
al., 2018) is administered \textbf{pre and post} session. Pre-administration is essential because
post-only sickness scores are uninterpretable without a baseline (Brown, Spronck \& Powell, 2022).
...
markedly in low light (Wang et al., 2026), so lighting is a controlled variable, not a convenience.
...
runs anywhere in the study. Instrumentation (the \verb|StudyLogger| telemetry component, condition
sequencer, marker scheduler, and post-session validator) is specified in the tooling appendix of the
operational protocol and summarised in Section 6.10.
...
its own source study, actively irritating 59 of 102 participants (Grogorick et al., 2018), and it also
carries the heaviest shader cost of any mode (Chapter 4). ChromaticCool loses to luminance-only
modulation in its own source study (Bailey et al., 2009). Testing either again here would repeat a
...
All twelve people tested (Section 6.11) were run in-headset on this apparatus. Task feel and timing
...
\textit{Figure 6.1: plan-view diagram of the Block A layout: participant, desk, task surface, tablets at
$\pm$35$^{\circ}$, focus-window extent.}
...
\begin{longtable}{p{0.12\textwidth}|p{0.48\textwidth}|p{0.32\textwidth}}
\caption{Block A conditions: vignette state, distractor reel present throughout.}\\
\hline
\textbf{Condition} & \textbf{Vignette} & \textbf{Tablets} \\
\hline
\endhead
\hline
NoFilter & Off (\verb|_DrIntensity = 0|; window painted but invisible) & Distractor reel \\
Filter & Hard Dark On (window live) & Distractor reel \\
\hline
\end{longtable}
...
world-locked panel rather than read through passthrough. Gate G1 (Section 6.8.2) confirms it performs
in range.
...
\subsection{Participant instructions (verbatim)}
...
panel matters. Each round lasts about three and a half minutes. There are two rounds. The rounds
...
attention automatically and would swamp the effect being measured (Yantis \& Jonides, 1984). It also
modulates the pixels underneath rather than covering them, so it keeps its own internal contrast even
where the filter has dimmed everything under it (Wallis, Dorr \& Bex, 2015). The marker itself is
...
region where driver gaze concentrates (Victor et al., 2005); 10–20$^{\circ}$ and 20–30$^{\circ}$, spanning
the published probe range for spare-capacity measurement (Jahn et al., 2005;
ISO 17488:2016); and beyond 30$^{\circ}$,
past the conventional useful-field-of-view extent (Ball \& Owsley, 1993). H2b's safety check pools hit
...
\section{Session Procedure and Pilot Study}
...
\subsection{Pilot study with pass/fail gates}
...
\begin{longtable}{p{0.14\textwidth}|p{0.22\textwidth}|p{0.30\textwidth}|p{0.26\textwidth}}
...
\textbf{H2b} & TOST on hit rate pooled across the whole field of view (paired), margin $\Delta$ = 10 pp (Lakens, 2017) & Bayesian paired test as an optional sensitivity check & Mean difference with 90\% CI against the margin \\
\hline
\end{longtable}
...
$N = 20$ collection concludes. The interim $N = 11$ results of Section 6.11 are consistent with row 1
(H1 supported; H2b passes on the safety side) but are not this table's confirmatory trigger. The
...
\subsection{Pre-registered exclusion and data-quality rules}
...
\textbf{Determinism.} Every distractor burst, marker onset, and task trial derives from versioned schedule
...
The sequencer additionally \textbf{asserts the required scene configuration at session start and refuses
...
window, and saliency boost do nothing without it). It logs the full configuration snapshot as
the session's first event rows.

\textbf{Logging with redundancy.} The telemetry component writes one CSV row per frame ($\sim$60 Hz) with
heartbeat semantics. An unbroken timestamp chain is itself evidence of integrity, and it flushes every
frame so that a crash loses under one second of data. Every event (condition boundaries, stimulus
onsets, presses, window lock, questionnaire start) is a labelled row. The \verb|scrcpy| screen recording
provides a redundant channel against which any disputed trial can be visually re-checked.

\textbf{Validate before the participant leaves.} A post-session validator checks timestamp continuity,
expected condition counts and durations, and per-condition event counts (for example, exactly 40
scored markers per Block B condition), returning PASS/FAIL while the participant is still available,
...
\textbf{Incident log.} A running incident file records every deviation: date, participant code, what
...
marks, tablet positions verified against a printed 35$^{\circ}$ angle template, build hash, storage, spare
...
Simulator sickness and dropout & Seated, no locomotion, $\sim$43 min headset time, mid-session break, VRSQ pre/post, stop rule, N + 4 recruitment \\
...
\section{Interim Results (N = 11)}

Recruitment continues toward the N = 20 target specified in Section 6.1.3. Twelve people have been
tested so far, across an initial pilot day and three further collection days. Each of the two
confirmatory blocks has eleven usable pairs. These are overlapping but not identical eleven-person
subsets, because one participant contributed a usable result to one block but not the other. What
follows is the interim analysis of those eleven pairs per block. It is reported honestly as interim
for three reasons: it is not the pre-registered confirmatory analysis of Section 6.9, the achieved
power at this N is well below 80\%, and recruitment is still ongoing. One participant's Block B data
is excluded under the pre-registered false-alarm rule (Section 6.9.4). Their filtered run recorded
539 presses that hit nothing, against 40 targets. An additional check confirmed this was
indiscriminate pressing rather than lucky detection: this person's scored catches landed further from
their target on average (median 1.82$^{\circ}$, worst decile 8.62$^{\circ}$) than everyone else's (median
1.59$^{\circ}$), against the 10$^{\circ}$ scoring radius.

\subsection{H1: Block A, workstation focus}

Accuracy on the memory task rose from 92.6\% with the vignette off to 96.3\% with Hard Dark on. That
is a gain of \textbf{+3.65 percentage points} (90\% CI +0.28 to +7.02, paired $t(10) = 2.42$,
$p = .036$, Wilcoxon $W = 8.0$, $p = .024$, $d_z = 0.73$). Nine of eleven participants improved. The
four people tested on pilot day 1 show a smaller gain than the rest: 97.2\% to 99.4\%, +2.20 points,
4 of 4 improved. The seven tested afterwards show 90.0\% to 94.5\%, +4.48 points, 5 of 7 improved.
This split fits a ceiling effect on day 1, not a weaker true effect: at 97.2\% only 2.8 points of
headroom remained to show any gain at all, so the task was made harder for the remaining sessions.
Achieved power at this N and effect size is 59\%. Around 17 participants would be needed for the
conventional 80\%. \textbf{H1 is supported at the interim N}, though the CI's lower bound sits close
to zero, so the effect's true size is not yet pinned down.

\subsection{H2b: Block B, pooled safety check}

Hit rate pooled across the whole field of view rose by \textbf{+3.9 percentage points} with SignPop
on. That figure is not independently reliable on its own (TOST 90\% CI $-4.9$ to $+12.6$ against the
pre-registered $\pm 10$-point margin). The worst case the data support is a loss of about five points,
comfortably inside the safety margin. \textbf{H2b is supported on the safety side}: peripheral
awareness was not lost, and if anything the pooled estimate is positive. The pooled figure only
answers the safety question, because it mixes the near-centre region the filter barely touches with
the far periphery it acts on fully. That is why H2a asks the finer-grained question below.

\subsection{H2a: Block B, the 20–30\textdegree{} band}

Hit rate broken out by eccentricity band shows a pattern, not a uniform shift. Under 10$^{\circ}$ the
change is $-1.1$ points, because the colour-pop gate is close to zero strength there and has nothing
to lift. From 10–20$^{\circ}$ the change is $+2.6$ points, where the gate sits at roughly 20\% strength.
\textbf{From 20–30$^{\circ}$ the change is +18.6 points} (40.0\% to 58.2\%, 90\% CI +2.5 to +34.7,
paired $t(10) = 2.57$, $p = .028$, Wilcoxon $W = 9.0$, $p = .030$, 9 of 11 improved, $d_z = 0.78$,
achieved power 64\%). Beyond 30$^{\circ}$ the change is $-1.5$ points: the gate is at full strength
there, but the target sits far enough into the periphery that a stronger signal buys nothing. Only the
20–30$^{\circ}$ band is individually reliable, and it is the only band whose direction agrees across
both independently collected phases: the pilot phase shows +25.4 points (4 of 4 improved), and the
seven tested afterwards show +14.7 points (5 of 7 improved). The other three bands flip sign between
phases, which is the signature of noise rather than a real effect. This is exactly the shape the
shader's own detection-gated design predicts. The gate has nothing to reveal below 10$^{\circ}$, and it
reveals the most beyond 30$^{\circ}$, but only in the 20–30$^{\circ}$ band is there both a strong effect
to reveal and an eye still able to use it. \textbf{H2a is supported at the interim N, specifically and
only in the band the mechanism predicts.} This is a secondary, exploratory analysis, not a substitute
for a confirmatory test at the final N.

Two further descriptive observations come from the 20–30$^{\circ}$ band. Briefly-visible markers, on
screen under two seconds, went from 2 of 36 caught (6\%) to 9 of 30 (30\%). And the share of catches
occurring while a marker sat in the 10–30$^{\circ}$ region rose from 41\% to 49\%, while the median angle
at the moment of the press barely moved (17.6$^{\circ}$ to 19.1$^{\circ}$). So participants were catching
more of what was already in front of them, not scanning further out to find it.
...
20–30$^{\circ}$ gain to the colour lift, the detection aperture, the saturation boost, or the darkened
surround individually. Ten end-of-session questionnaires (Section 6.7) show a divergence worth
flagging rather than resolving here. Perceived focus benefit is strongly positive, 47 of 50 responses
favouring the effects, and the three items on holding attention, looking away, and knowing where to
look were answered the same way by all ten participants. Perceived awareness cost splits close to
evenly, 29 of 50, so about half the responses report feeling cut off from the surroundings or
noticing side events late. Visual comfort sits at 30 of 40 across its four answered items. The
subjective impression of lost awareness is not supported by the behavioural data above, and it is
discussed in Chapter 7. Achieved power throughout this section is 59\% for H1 and 64\% for
the 20–30$^{\circ}$ band. This means a null result at this N would not have been informative either
way. The results reported here happened to clear their thresholds regardless, which is not the same
claim as having been powered to find them.

\section*{References (this chapter)}

\refitem{Bailey, R., McNamara, A., Sudarsanam, N., \& Grimm, C. (2009). Subtle gaze direction. \textit{ACM
Transactions on Graphics, 28}(4). \url{https://doi.org/10.1145/1559755.1559757}}

\refitem{Ball, K., \& Owsley, C. (1993). The useful field of view test: a new technique for evaluating
age-related declines in visual function. \textit{Journal of the American Optometric Association, 64}(1),
71–79. PMID: 8454831.}

\refitem{Caine, K. (2016). Local standards for sample size at CHI. In \textit{Proceedings of CHI 2016} (pp.
981–992). ACM. \url{https://dl.acm.org/doi/10.1145/2858036.2858498}}

\refitem{Cheng, Y., Yin, H., Yan, Y., Gugenheimer, J., \& Lindlbauer, D. (2022). Towards understanding
diminished reality. In \textit{Proceedings of CHI 2022}. ACM.
\url{https://dl.acm.org/doi/10.1145/3491102.3517452}}
...
\refitem{Brown, P., Spronck, P., \& Powell, W. (2022). The simulator sickness questionnaire, and the
erroneous zero baseline assumption. \textit{Frontiers in Virtual Reality, 3}:945800.
\url{https://doi.org/10.3389/frvir.2022.945800}}
...
\refitem{ISO (2016). \textit{ISO 17488:2016 — Road vehicles — Transport information and control systems —
Detection-response task (DRT) for assessing attentional effects of cognitive load in driving.}}

\refitem{Jahn, G., Oehme, A., Krems, J. F., \& Gelau, C. (2005). Peripheral detection as a workload measure in
driving: effects of traffic complexity and route guidance system use in a driving study.
\textit{Transportation Research Part F, 8}(3), 255–275. \url{https://doi.org/10.1016/j.trf.2005.04.009}}

\refitem{Kim, H. K., Park, J., Choi, Y., \& Choe, M. (2018). Virtual reality sickness questionnaire (VRSQ):
motion sickness measurement index in a virtual reality environment. \textit{Applied Ergonomics, 69},
66–73. \url{https://www.sciencedirect.com/science/article/abs/pii/S000368701730282X}}

\refitem{Lakens, D. (2017). Equivalence tests: a practical primer for t tests, correlations, and
meta-analyses. \textit{Social Psychological and Personality Science, 8}(4), 355–362.
\url{https://doi.org/10.1177/1948550617697177}}

\refitem{McLaughlin, A. C., et al. (2025). Cognitive aid design using diminished reality to support selective
attention by reducing distraction. \textit{Human Factors, 67}(9), 937–961.
\url{https://journals.sagepub.com/doi/10.1177/00187208251325169}}
...
see-through displays and natural human vision. \textit{arXiv:2601.02805.} \url{https://arxiv.org/pdf/2601.02805}}
...
\refitem{Victor, T. W., Harbluk, J. L., \& Engström, J. A. (2005). Sensitivity of eye-movement measures to
in-vehicle task difficulty. \textit{Transportation Research Part F, 8}(2), 167–190.
\url{https://doi.org/10.1016/j.trf.2005.04.014}}

\refitem{Tsandilas, T., \& Casiez, G. (2024). The illusory promise of the Aligned Rank Transform. \textit{Journal of
Visualization and Interaction} (under review). \url{https://statransform.github.io/jovi/}}

\refitem{Wobbrock, J. O., Findlater, L., Gergle, D., \& Higgins, J. J. (2011). The aligned rank transform for
nonparametric factorial analyses using only ANOVA procedures. In \textit{Proceedings of CHI 2011} (pp.
