...
\textbf{Gate outcomes.} G2 passed at pilot; across the full sample the NoFilter baseline converged
lower, settling at 50.15\%. G1 was not cleared. Block A accuracy sat on the
...
rather than re-piloting, under the rule in row 5 of the decision table. Both shortfalls run in the
direction that preserves headroom: the lower Block B baseline removes any ceiling explanation for the 20--30$^{\circ}$ band
...
rehearsals rather than data collection, so the outcome is recorded observationally.
...
The head-turn analysis, the interview coding, and the VR-experience subgroup re-estimation were
...
Order and learning effects & Alternating condition order within each block, balanced in Block A and unevenly realised in Block B, where twelve of seventeen pairs ran NoFilter first; matched sheets/segments; equal trial counts. Block order is counterbalanced across participants as well as within each block, so neither block is systematically second and cross-block fatigue cannot load onto one of them; about half the sample ran the driving block first \\
