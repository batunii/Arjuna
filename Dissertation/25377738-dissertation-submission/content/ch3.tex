...
Both concerns seem likely to strengthen rather than weaken over time, so research that assumes
future read-modify-write access to system passthrough is betting against the platform vendors'
incentives. This dissertation makes the opposite bet. It treats the constraint as permanent and asks
what can be built inside it, which is what gives the resulting design space its claim to durability.
...
1280$\times$960 per camera at $\sim$85–90$^{\circ}$ horizontal FOV vs the $\sim$110$^{\circ}$ OS view. Added latency. Binocular-fusion burden (Chapter 4). Sustained on-device processing is thermally bounded &
...
Tier 1 needs precise description, because some adjustment of the system passthrough layer is
possible and overlooking it would overstate the constraint. Meta's Passthrough
...
The price is paid in four currencies. \textbf{Resolution and field of view}: the accessible feed is
1280$\times$960 per camera covering roughly 85–90$^{\circ}$ horizontally, against the OS
...
reprojection, does not exhibit. \textbf{Binocular fusion}: the cameras are forward-facing and
offset from the eyes rather than eye-aligned, so a Tier-3 re-render must be resampled into a
form two eyes can fuse; the naive approach measurably fails (Chapter 4). Several of the modes
here, including the evaluated one, sample a single camera, which makes this the operative
...
\textbf{The composite itself was never placed in front of a participant.} Hard Dark was evaluated at
the workstation and SignPop in the video scene, so no session put a shader-processed camera periphery
around a native passthrough window. The composite is validated on the device rather than on people
(Chapter 4, Section 4.11), and no claim here rests on how it is experienced.
...
Chapter 2, Section~\ref{sec:priorart}. Three verdicts matter here.
...
fusion sampling that makes a single-camera feed binocularly viewable. An alpha-blended dark overlay, by
itself, is just compositing, and is not claimed as novel. The claim is worded around the design
space rather than the single technique. If a concurrent system were to surface, the contribution
would remain a characterised design space with an independently built implementation.
...
latency, fusion and heat. The modes carried into the user study span the space this stratification
creates, the hybrid
