...
published or shipped precedent found. The architecture's two central trades were then measured on
the device itself: the camera-free dark modes cost nothing over an empty passthrough scene, and the
...
The second half is answered by the study, at seventeen workstation pairs and fifteen driving pairs,
two short of the pre-registered twenty, with the decision table applied as committed. The answers:
a 3.09-point accuracy gain on a real-hardware task, real, significant, and adequately powered, but
smaller than the pre-set half-the-errors bar, so claimed as a bounded benefit under the table's
fourth row rather than at the pre-set size; peripheral awareness that was not lost, the safety
hypothesis's refuting observation never occurring; and a detection benefit concentrated precisely in
the 20--30$^{\circ}$ band the system's own detection-gated geometry predicts, supported at 84\%
power. "Inconclusive" was designed out of the reachable outcomes, and the outcome that did occur is
...
platform-policy finding rather than an engineering one, and it shapes what DR research is possible on
consumer hardware more than any algorithm does.
...
so designs that keep a working path at every tier are robust to that movement and designs premised on
a single capability are not.
...
\item \textbf{The sample is eighteen, not the pre-registered twenty.} Collection ended at the
  dissertation deadline. The primary hypothesis and the 20--30$^{\circ}$ band sit at 82\% and 84\%
  achieved power (one-tailed), at or above the design's target, which is why the analysis stands,
  but every interval is wider than the design intended, the pooled Block B estimate remains at 50\%
  power with a response-criterion qualification (Chapter 5, Section 5.7.2), and the primary effect's
  size could not be pinned against its smallest effect size of interest, which is exactly what the
  two missing participants were for.
\item \textbf{Instrumentation gaps.} A per-condition NASA-TLX was specified but never administered,
  so the study cannot say whether the vignette made the task feel easier as well as produce fewer
  errors, and the distractor reel's cost is taken from the published literature \citep{ai2025} rather
  than verified internally.
...
modulation demonstrated by \citet{sutton2022}. Critically, the \textbf{human-factors findings do
transfer}, because what peripheral diminishment does to attention is a property of the human visual
system rather than of the display that implements it. The engineering is platform-bound. The
psychology is not. A replication of the Chapter 5 paradigm on a dimming-capable OST device would be
the cleanest possible test of that transfer.
...
collection, is not standard practice at masters-project scale, where underpowered multi-hypothesis
designs that can only end inconclusively remain common. The apparatus costs little, since its
...
The system exists and runs on target hardware. The hypothesis was tested, on seventeen workstation
pairs and fifteen driving pairs, and answered under the rules committed to before the data existed:
a real focus benefit, claimed at its bounded size rather than its hoped-for one; peripheral
awareness kept; the mechanism confirmed exactly where its geometry said to look. The question was
asked properly, on the hardware people actually own, and that is the standard the answer inherits.
