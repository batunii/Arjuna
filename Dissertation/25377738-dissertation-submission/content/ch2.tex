...
This chapter argues one line of reasoning rather than surveying bodies of literature side by side. Is
the problem real, what has already been tried, has removing things instead of adding them been tried
and did it work, can any of this be built on hardware people actually own, and how should the result
be tested. A study earns its place here when it establishes that the problem is real, when it
justifies a value this system actually uses, or when it supplies a method the evaluation adopts. Work
that merely neighbours the topic is left to the surveys that cover it.
...
intensity, colour opponency, or orientation, differs most from their neighbourhood. Three
consequences recur throughout the design. First, \textit{contrast is the master variable}, so any
manipulation raising local contrast attracts gaze and any manipulation flattening it repels gaze.
...
event happens before, and regardless of, the observer's intentions. A periphery whose salience has
been flattened stops issuing capture events at all, which is the mechanism the dark modes exploit.
...
Where salience explains \textit{how} the periphery captures attention, Cognitive Load Theory explains
...
That framing also names the project's central risk, which is why this dissertation pairs two
hypotheses rather than one. Attention capture by peripheral events is not a design flaw of the human
visual system. It is a safety mechanism. Suppressing it trades focus against situational awareness, a
tension documented empirically in the DR literature \citep{murph2021,mclaughlin2025} and formalised
in Chapter 5 as an equivalence-tested safety hypothesis rather than an afterthought. The evaluation
design refuses to measure benefit without simultaneously measuring cost.

What was missing until recently was the distractor-cost baseline in immersive settings. That has now
been supplied. Peripheral visual distractors in a virtual classroom significantly increase both
commission errors (1.33 $\rightarrow$ 3.15) and omission errors on a Go/No-go continuous performance
task, while leaving mean reaction time unchanged \citep[N = 66]{ai2025}. That single result does
...
law above. Two results from this family set hard constraints on the present design.
...
Slow ramps are the one escape from the noticeability trade-off. \citet{hata2016} showed that a
visual change introduced gradually enough stays below the awareness threshold while still biasing
gaze, because the guidance threshold sits below the awareness threshold. This is the perceptual
...
video-see-through headset. This dissertation can be read as the first software-defined visual noise
cancellation system on consumer video-passthrough hardware, where each mode is a cancellation profile
and the profiles differ in what they classify as noise. The analogy also imports a useful discipline
...
Peripheral vision is not blurry central vision. It is a differently tuned system that notices
contrast loss and motion change most, and chromatic detail loss least. One perceptual finding is
...
local contrast after blurring roughly doubles the tolerable blur radius. That chain also explains,
mechanistically, Grogorick et al.'s irritation finding, since naive whole-periphery blur announces
itself through contrast collapse. It is why the dark modes attenuate luminance, which the periphery
tolerates, rather than blurring structure, which it resents.
...
no published study was found that shader-processes their passthrough region-selectively for
attention. Researchers with the capability have not asked this question, while the consumer platforms
that provoke it lack the capability.
...
\citep{brown2022}. The study administers no per-condition workload instrument, though the NASA-TLX
\citep{hart1988} is the field standard, and Chapter 6 records what that costs. Finally, equivalence
...
One applied result matters structurally, because this dissertation uses driving \textit{footage} as a
stimulus context. AR cueing of roadside hazards improves response time \textit{without} degrading
detection of non-target objects \citep[N = 27]{rusch2013}, an early demonstration that guidance
benefit and situational-awareness cost are separable, which is exactly the separation this
dissertation's secondary hypotheses formalise. That literature motivates the scenario aesthetics and
supplies the salience-engineering precedent for ColorPop's red, orange and green hierarchy. It
licenses nothing further: per Chapter 1, no claim about driving, drivers, or road safety is made
