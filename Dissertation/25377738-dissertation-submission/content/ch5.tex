...
frozen protocol after a formal pass/fail pilot. Collection ended at \textbf{eighteen people tested},
...
and just as importantly to be able to determine that it \textit{does not}. A study that can only
confirm is not an experiment. It is a demonstration. Two design choices produce that defect: an
underpowered between-subjects comparison, and safety hypotheses phrased as bare nulls, since a
non-significant difference in a small sample is evidence of nothing.

The design removes the possibility of an inconclusive outcome by reframing the question around an
...
Each participant contributes 140 scored task events per Block A condition and 40 marker events per
Block B condition, so the seventeen analysed workstation pairs contribute 4,760 scored Block A
trials.
...
documented by \citet{mclaughlin2025} and \citet{murph2021}. In a monitoring context, losing more than
one detection event in ten to the overlay is an unacceptable awareness cost. Equivalence testing
...
\citep{brown2022}. Across the sessions run to date the stop rule has not been triggered and no
...
\textbf{Filter}, Hard Dark on. Each lasts 4 minutes 40 seconds, which is 140 trials at a 2.0 s
stimulus onset asynchrony, run once each per participant in counterbalanced order. Before the first
...
17$^{\circ}$ of travel on average, so scoring against a marker's average position would misassign a
meaningful share of presses to the wrong analysis band. Every press is instead scored against the
marker's reconstructed position, from the recorded scene track, at the exact instant of the press.
...
hypotheses are revealed. It carries 23 rated items across three constructs, perceived focus benefit,
perceived awareness cost and visual comfort, each mixing positively and negatively keyed items to
counter acquiescence bias. Because it asks for one considered opinion of the whole session, it cannot
...
it when the dissertation deadline arrived. That is the study's largest protocol deviation and it is
stated first. The analysis proceeds on a specific ground rather than convenience: the 20-participant
...
ledger. Three participants are excluded from Block B under documented rules, never inferred ones: P3
for 539 false alarms on the filter run with provably looser hits (Mann-Whitney $p = .0301$), P9 by
experimenter decision on the day, and P43 for false-alarm counts of 81 and 54 across the two arms,
the highest in the set by a wide margin, on the same response-validity grounds as P3. A fourth,
P15, had their own Block B excluded for 206 false alarms across 246 presses and is analysed jointly
with P16 as one participant. Block A is retained for P3, P9 and P43, since the exclusions concern
the marker task alone.

Provenance facts belong with the numbers rather than in a footnote. First, Block A is pooled
across the four task versions used during the study. The versions differ, and a Kruskal-Wallis test
across them is significant ($H = 16.36$, $p = .0010$), but the difference is the deliberate v4 change
that added lures and switched the distractor reel in response to a 95--100\% ceiling on v3. Within
the test versions the only timing difference is a 1.8 s against a 2.0 s stimulus onset asynchrony,
and median correct reaction time across every run sits between 630 and 950 ms, so no participant is
response-limited under either and a 200 ms difference cannot plausibly drive an accuracy gap. Second,
and less comfortably, \textbf{one of the 34 Block A values is experimenter-reported rather than
measured}: P4's no-filter accuracy of 98.4\% has no exported CSV. It is retained because the
assignment is documented, but it is the only such value and every Block A figure inherits that
qualification. Dropping it changes nothing ($p = .0175$ against $.0212$, $d_z$ unchanged).

Three protocol deviations from the 2026-08-06 and 2026-08-07 sessions are reported here because the
protocol is pre-registered. The build changed between the 7 August sessions. Two participants (P41,
P43) removed the headset after their final block instead of pressing the completion control, so the
session ledger lacks its final block row for each; both blocks are verified complete against the
task's own trial-by-trial log, so no data is missing, only the ledger's bookkeeping. And P25 ran
Block A twice: the re-run pair, taken six minutes apart so fatigue and time of day are matched, is
canonical, and its no-filter arm re-used a slot and seed the participant had seen about three hours
earlier. That repeat is not a familiarity confound, since a 1-back task does not reward sequence
knowledge and the repeat scored \textit{worse} (86.3\% against 99.3\%), but the unexplained drop on
identical trials is recorded in the incident log.

\subsection{H1: workstation focus}

Accuracy on the memory task rose from 92.30\% (SD 6.56) with the vignette off to 95.39\% (SD 4.17)
...
\caption{Block A per-participant accuracy, vignette off versus Hard Dark on (N = 17). P4 is drawn
dashed because its vignette-off value is experimenter-reported rather than measured.}
\label{fig:h1}
\end{figure}
...
at that sample size could carry; the one strong dissenter on focus benefit is also the participant
whose unexplained Block A re-run drop is recorded in Section 5.7. Figure~\ref{fig:esq} shows the
three constructs side by side.
...
changed is a different experience from missing something in it, and every DR manipulation studied to
date is noticed \citep{sutton2022}. "I can tell my vision has been altered" and "I failed to detect
