...
simulated video scene and evaluates the concept under the ideal-context assumption.
...
and gender distributions cannot be reported; the inclusion criteria and recruitment route
...
sample. Across the seventeen usable pairs, twelve ran NoFilter first and five ran Filter first, and
set A was the scored set under Filter for eleven of the seventeen.
...
a median 17$^{\circ}$ of travel, so scoring against a marker's average position misassigns a
...
The first provenance fact is the study's \textbf{one protocol deviation}. Block A pools a short and a long form of
...
+0.89 to +5.25, paired $t(18) = 2.95$, $p = .0085$; Wilcoxon $W = 23.0$, $p = .007$; $d_z = 0.678$).
...
\label{tab:bands} \\
\hline
\textbf{Band} & \textbf{Off, pooled} & \textbf{On, pooled} & \textbf{Difference (per participant)} & \textbf{95\% CI, $p$} \\
\hline
\endfirsthead
\hline
\textbf{Band} & \textbf{Off, pooled} & \textbf{On, pooled} & \textbf{Difference (per participant)} & \textbf{95\% CI, $p$} \\
...
\endhead
\hline
$<$10\textdegree{} & 75.0\% & 78.3\% & $-3.68$ pp & $[-13.8, +6.4]$, $p = .452$ \\
10--20\textdegree{} & 40.8\% & 50.2\% & $+9.74$ pp & $[-2.4, +21.9]$, $p = .108$ \\
\textbf{20--30\textdegree{}} & \textbf{38.9\%} & \textbf{59.6\%} & \textbf{$+20.82$ pp} & \textbf{$[+7.8, +33.8]$, $p = .004$} \\
$>$30\textdegree{} & 54.4\% & 55.4\% & $+0.98$ pp & $[-7.2, +9.1]$, $p = .802$ \\
\hline
\end{longtable}
...
the same way at +9.7 points, but its 95\% interval spans zero and its tests do not
...
fixed against the driving-attention literature rather than against these results \citep{victor2005,
jahn2005}. The measured benefit follows the filter's ramp: absent
...
gives the paired per-participant difference with its 95\% interval, which is the estimator the tests
